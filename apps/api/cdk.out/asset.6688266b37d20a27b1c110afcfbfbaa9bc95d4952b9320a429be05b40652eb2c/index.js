...
// asset-input/apps/api/mappers/cardMappers.ts
var safeParseJSON = (json, id) => {
  try {
  ...
  async createCard(card) {
    ...
    await client.send(
      new import_client_dynamodb.PutItemCommand({
        TableName: tableName,
        Item: (0, import_util_dynamodb.marshall)(record)
      })
    );
  }
  async getCardById(id) {
...
var handler = async (event) => {
  try {
    ...
    switch (event.info.fieldName) {
      case "getCardById": {
        return await service.getCardById(event.arguments.id);
      }
      case "getAllByType": {
